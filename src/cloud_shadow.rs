...
impl CloudShadowState {
    pub fn new(device: &wgpu::Device, queue: &wgpu::Queue) -> Self {
        ...
        noise3.set_frequency(Some(3.2));

        let mut pixels = vec![0u8; (size * size) as usize];
        let tau = std::f32::consts::TAU;
        let radius = 1.0;  // Torus radius - controls feature density

        for y in 0..size {
            for x in 0..size {
                // Map pixel coords to angles on torus
                ...
                // Sample 3 octaves using pairs of 4D coords via 2 x get_noise_3d
                // FastNoiseLite doesn't have 4D, so use get_noise_3d(nx, ny, nz) + offset by nw
                let o1a = noise1.get_noise_3d(nx, ny, nz + nw);
                let o1 = o1a * 0.5 + 0.5;

                let o2a = noise2.get_noise_3d(nx, ny, nz + nw);
                let o2 = o2a * 0.5 + 0.5;

                let o3a = noise3.get_noise_3d(nx, ny, nz + nw);
                let o3 = o3a * 0.5 + 0.5;
    ...
    pub fn update(&mut self, dt: f32, wind: &Vec2, elapsed: f32) {
        self.offset += *wind * dt * 0.04;
        self.coverage = 0.45 + 0.10 * (elapsed * 0.05).sin();
    }
}